...
namespace
{
	const char* const WINDOW_TITLE = "Shape Meshes Collection"; // Macro for window title
...
int main(int argc, char* argv[])
{
	...
	gCameraPosition = glm::vec3(0.0f, 6.5f, 10.0f);
	gCameraFront = glm::vec3(0.0f, -2.5f, -3.0f);
...
void URender()
{
	...
	view = glm::lookAt(gCameraPosition, gCameraPosition + gCameraFront, gCameraUp);

	// Creates a orthographic projection
	//projection = glm::ortho(-5.0f, 5.0f, -5.0f, 5.0f, 0.1f, 100.0f);
	projection = glm::perspective(glm::radians(50.0f), (GLfloat)WINDOW_WIDTH / (GLfloat)WINDOW_HEIGHT, 0.1f, 100.0f);
	...
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-4.0f, 1.0f, 2.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
